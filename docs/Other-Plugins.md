...
The separator must be a normal lore line. Avoid plugin-specific formatting that transforms or removes the separator before the item reaches Bukkit lore.
...
For menu or GUI items, avoid using the same click type for both PageLore and the menu action. If a menu already uses middle click, move PageLore full-lore mode to another `ClickType`.